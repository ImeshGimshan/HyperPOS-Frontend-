...
// Imports : ( useState , useEffect ) , ( Eye , SlidersHorizontal ) , ( userData )
...
function FilterModal ( { onClose, onApply, currentFilters, roles } ) {
  ...
  const handleApply = ( ) => {
    ...
    onApply ( { name, email, phone, role, status } );
...
function UserPage ( ) {
  ...
  const filteredData = Array.isArray(userData) 
    ? userData.filter ( ( user ) => {
      ...
      return matchesSearch && matchesName && matchesEmail && matchesPhone && matchesRole && matchesStatus;
  ...
  return (
    ...
    <div className = "p-6">
      <h1 className = "text-3xl font-bold mb-6 text-purple-900 text-center">User Management</h1>
          ...
          <table className = "min-w-full text-sm text-left">
            ...
            <tbody>
              { filteredData.length > 0 ? (
                ...
                <tr>
                  <td colSpan = "7" className = "p-3 text-center font-medium text-gray-500">No users found</td>
                </tr>
              )}
            </tbody>
          </table>
