      ...
      <div className = "bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative">
        ...
        <div className = "px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
          <div className = "w-full text-center">
            <h2 className = "text-xl sm:text-2xl font-bold text-purple-900">User Details</h2>
          </div>
          <button
            onClick = { onClose }
              ...
              <div className = "bg-purple-50 p-3 sm:p-4 rounded-xl">
                ...
                <div className = "grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div className = "flex flex-col">
                    ...
                    <span className = "p-2 bg-white rounded-md shadow-sm">{ user.id }</span>
                  </div>
                  <div className = "flex flex-col">
                    <span className = "font-medium text-gray-600">Status</span>
                    <span className = {`p-2 bg-white rounded-md shadow-sm ${user.isActive ? "text-green-600" : "text-red-600"}`}>
                      { user.isActive ? "Active" : "Inactive" }
                    </span>
                  </div>
                  <div className = "flex flex-col col-span-1 sm:col-span-2">
                    <span className = "font-medium text-gray-600">Name</span>
                    <span className = "p-2 bg-white rounded-md shadow-sm">{ user.name }</span>
                  </div>
                  <div className = "flex flex-col">
                    <span className = "font-medium text-gray-600">Username</span>
                    <span className = "p-2 bg-white rounded-md shadow-sm">{ user.username }</span>
                  </div>
                  <div className = "flex flex-col">
                    <span className = "font-medium text-gray-600">Role</span>
                    <span className = "p-2 bg-white rounded-md shadow-sm">{ user.role }</span>
                  </div>
                </div>
              </div>
              ...
              <div className = "bg-blue-50 p-3 sm:p-4 rounded-xl">
                ...
                <div className = "grid grid-cols-1 gap-3 text-sm">
                  <div className = "flex flex-col">
                    ...
                    <span className = "p-2 bg-white rounded-md shadow-sm">{ user.email || "Not provided" }</span>
                  </div>
                  <div className = "flex flex-col">
                    <span className = "font-medium text-gray-600">Phone</span>
                    <span className = "p-2 bg-white rounded-md shadow-sm">{ user.phone || "Not provided" }</span>
                  </div>
                </div>
              </div>
              ...
              <div className = "bg-amber-50 p-3 sm:p-4 rounded-xl">
                ...
                <div className = "grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div className = "flex flex-col">
                    ...
                    <span className = "p-2 bg-white rounded-md shadow-sm">{ formatDate(user.createdAt) }</span>
                  </div>
                  <div className = "flex flex-col">
                    <span className = "font-medium text-gray-600">Last Updated</span>
                    <span className = "p-2 bg-white rounded-md shadow-sm">{ formatDate(user.updatedAt) }</span>
                  </div>
                </div>
              </div>
            </div>
...
function UserPage ( ) {
  ...
  const filteredData = Array.isArray(userData) 
    ? userData.filter ( ( user ) => {
      ...
      // Apply filters
      const matchesName = !filters.name || (user.name && user.name.toLowerCase().includes(filters.name.toLowerCase()));
      const matchesUsername = !filters.username || (user.username && user.username.toLowerCase().includes(filters.username.toLowerCase()));
      ...
      return matchesSearch && matchesName && matchesUsername && matchesEmail && matchesRole && matchesStatus;
  ...
  return (
    ...
    <div className = "p-4 sm:p-6">
      <h1 className = "text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 text-purple-900 text-center">User Management</h1>
